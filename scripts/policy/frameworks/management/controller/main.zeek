...
event Management::Controller::API::get_id_value_request(reqid: string, id: string)
	...
	for ( name in g_instances )
		...
		Management::Log::info(fmt("tx Management::Agent::API::get_id_value_request %s to %s", areq$id, name));
		Broker::publish(agent_topic, Management::Agent::API::get_id_value_request, areq$id, id);
		}
	}
